...
- 2026-08-03: Debugged Supabase keepalive failure. Root cause was the repo-root GitHub Actions keepalive workflow being auto-disabled as `disabled_inactivity` after 60 days without public-repo activity; the latest successful run was 2026-07-29 17:58 UTC. Added a Cloudflare Workers Cron keepalive project under `ops/supabase-keepalive-worker/`, strengthened the GitHub workflow as backup only, and added `tools/check-supabase-keepalive.ps1` for future DNS/API/workflow diagnostics. Current Supabase project host `ztpftxwfcppsqosilssq.supabase.co` resolves NXDOMAIN locally, so the Supabase dashboard likely needs a manual resume before keepalive can succeed again.
