...
- 2026-04-29: Startup load pass: `loadPlayground()` now starts in parallel with player assets and waits for BVH only before the minimal ground-mesh tree build; initial BVH build was narrowed to `groundMeshes` and the full-scene remainder still finishes during loading via `schedulePlaygroundBVHBuild()`. Added baked JSON hooks for `field/playable_layer_baked_v1.json` and `field/river_bounds_baked_v1.json` with localStorage fallback, switched core GLB loads to a shared Cache API helper, moved `clean` badge out of `#game-controls` so it can overlay the frame, raised `PAINT_FILL_GOAL` to 20, and nudged both desktop/mobile `E` frame+button up by 0.2%.
...
- Added a quantized playground scene variant (ield/playground_optimized.glb) using KHR_mesh_quantization. Size dropped from 17.0 MB to 11.7 MB and estimated GPU geometry footprint from ~16.0 MB to ~8.9 MB. Loader now prefers the optimized file and falls back to the original playground.glb if loading fails; preload link was updated to match. Also froze static transforms for playground/water/graffiti/flower groups and requested high-performance GPU mode on the renderer.
...
- 2026-05-01: Startup optimization pass: only critical ground meshes now build BVH inside loadPlayground, while the rest of the playground defers to schedulePlaygroundBVHBuild after floor selection. loadPlayground also now reuses a single root bbox and a single mesh traversal for material prep + pillar detection + ground mesh collection. Added baked playable-layer data at `urban-legend-framework/field/playable_layer_baked_v1.json`, and verified a clean reload now logs `[floor] loaded baked playable layer` instead of doing the full floor scan.
- 2026-05-30: Fixed a repeated manual UI-export mistake where `RIVER_REGION_DEFAULTS` / `SCENE_OBJECT_DEFAULTS` were pasted below `UI_LAYOUT_DEFAULTS`, causing duplicate `const` declarations. Added `tools/publish-ui-export.ps1` and repo-root `PUBLISH_UI_EXPORT.cmd` so future UI exports can be applied from the clipboard, validated, committed, and pushed without GitHub Desktop staging.

- 2026-08-03: Debugged Supabase keepalive failure. Root cause was the repo-root GitHub Actions keepalive workflow being auto-disabled as `disabled_inactivity` after 60 days without public-repo activity; the latest successful run was 2026-07-29 17:58 UTC. Strengthened the GitHub workflow with retries and a weekly heartbeat commit using GitHub's built-in `GITHUB_TOKEN`, so the schedule keeps repo activity alive without Cloudflare or extra external tokens. Added `tools/check-supabase-keepalive.ps1` for future DNS/API/workflow diagnostics. Current Supabase project host `ztpftxwfcppsqosilssq.supabase.co` resolves NXDOMAIN locally, so the Supabase dashboard likely needs a manual resume before keepalive can succeed again.
